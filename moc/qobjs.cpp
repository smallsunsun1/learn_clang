...
bool QBJS::Parse(llvm::yaml::Node *Node, QBJS::Value &Root) {
  if (!Node)
    return false;
  if (llvm::yaml::SequenceNode *Array =
    ...
    return true;
  } else if (llvm::yaml::MappingNode *Object =
    ...
    for (llvm::yaml::MappingNode::iterator KVI = Object->begin(),
      ...
      if (!Parse(Value, Root.Props[KeyString->getValue(Storage).str()]))
        return false;
    }
    return true;
  } else if (llvm::yaml::ScalarNode *Scal =
